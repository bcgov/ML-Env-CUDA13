...
- Run tests:
   - Core GPU verification (required):
      - WSL: `python test_torch_cuda.py`
      - Windows/PowerShell: `python test_pytorch.py`
   - Additional checks (optional): `python test_tensorflow.py`, `python test_xformers.py`, `python test_llama_cpp.py`
   - Regenerate test scripts without installing packages:
      - WSL: `bash setup_ml_env_wsl.sh --regen-tests-only`
      - PowerShell: `./setup_ml_env_full.ps1 -RegenTestsOnly`
